      ...
      <div className="p-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          <div className='flex items-center'>
            <Button onClick={prevMonth} variant="ghost" size="icon">
             ...
             <h2 className="text-2xl font-bold text-center w-64 capitalize">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:w-auto">
            ...
            >
              {Object.entries(turnIcons).map(([turn, icon]) => (
                <ToggleGroupItem key={turn} value={turn} aria-label={turn} className='flex gap-1'>
                  {icon} {turn}
                </ToggleGroupItem>
              ))}
            </ToggleGroup>
        ...
        <div className="grid grid-cols-7 gap-1">
          {Array.from({ length: startingDayIndex }).map((_, i) => (
            <div key={`empty-${i}`} className="border rounded-lg bg-muted/30 min-h-[120px]"></div>
          ))}
          ...
          {days.map(day => {
            ...
            return (
              <div
                ...
                className={cn(
                  'border rounded-lg p-2 min-h-[120px] cursor-pointer transition-all duration-200',
