...
export function PanelEventCard({ event }: PanelEventCardProps) {
  return (
    <div
      className="p-3 rounded-lg text-sm border-l-4 bg-slate-900/70"
      style={{ borderColor: event.color}}
    >
      <div className="flex justify-between items-start mb-2">
        <p className="font-bold text-base text-white pr-2">{event.name}</p>
      ...
      <div className="space-y-1 text-slate-300">
        ...
        <div className="flex items-center gap-2">
            <Video className="h-3.5 w-3.5 flex-shrink-0 text-amber-400" />
