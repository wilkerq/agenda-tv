 ...
 *
 * - importAlegoAgenda - Fetches, parses, and saves events from the Alego website for the next 30 days.
 */
...
const importAlegoAgendaFlow = ai.defineFlow(
  ...
  async () => {
    ...
    // Fetch for the next 30 days from today
    ...
    try {
        ...
        for (const event of apiEvents) {
            ...
            // Skip invalid or past events right away
            if (!isValid(eventDate) || !event.title || !event.location || eventDate < startOfDay(today)) {
                continue;
            }
    ...
    // Fetch existing events for the relevant period to avoid duplicates
    const q = query(
        eventsCollection,
